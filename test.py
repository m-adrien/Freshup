...
parser.add_argument("-F", "--force", help="Force the installation on other distribution BE CAREFUL",
                    action="store_false")
parser.add_argument("-R", "--restart", help="Restart the system at the end of the programm (init 6)",
                    action="store_false")
args = parser.parse_args()

# -------------------------------------------- Verification de conf.json -------------------------------------------- #
# Encodage du regex pour determiner ip dans la variable regip
regip = re.compile("^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$")
...
def verifkey3(key):
    """Vérifie le contenue d'une clé pour que sa valeur soit égale à DMZ, LAN ou WAN si non --> exit status = 3"""
    if (key != 'DMZ' and key != 'LAN' and key != 'WAN'):
